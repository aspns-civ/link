...
// TODO: Test that it also fetches confirmed_missing_receipt
...
//
// In this case we wait until the eth_tx has all of its attempts broadcast
// before some cutoff threshold, then mark it as fatally errored (never sent).
//
...
UPDATE eth_txes
SET state='fatal_error', nonce=NULL, error=?, broadcast_at=NULL
WHERE id IN (
	SELECT eth_txes.id FROM eth_txes
	LEFT OUTER JOIN eth_tx_attempts ON eth_txes.id = eth_tx_attempts.eth_tx_id
	WHERE state = 'confirmed_missing_receipt'
	HAVING max(eth_tx_attempts.broadcast_before_block_num) < ?
)
